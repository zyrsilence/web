...
window.onload=function(){
	...
	yc.addEvent("btncomfirm1","click",function(){
			if(!yc.$('flabel').value ||!yc.$('furl').value){
				...
				var options={
			    "method":"POST",
			    "send":yc.serialize(document.myform1)+"fid=",/////////////////////////啊啊啊啊啊不会//////////////////////////////
			    "jsonResponseListener":function(json){
					if(json.code=0){
					alert("添加失败");
					}
	...
	//var colorarr=['#FECCCD','#A4C0E8','#FECCCD','#A4C0E8','#FECCCD','#A4C0E8']
	//8.6号版本  胎死腹中
	//var colorarr1=[['#daf0df','#b3dbbf','#85b8a1','#65918e','#edc374','#f0ecaf'],[]]
	//标签名查询网址  #ff9999
			...
			"jsonResponseListener":function(json){
				...
				for(var i=0;i<tagarr.length;i++){
					...
					if(reg.test(tagarr[i]['furl']) && tagarr[i]['flabel']&& !check.test(tagarr[i]['flabel'])){
						...
						nav.style.backgroundColor=colorarr[Math.floor(Math.random()*5)];
						pen.className="pen";
	...
	yc.addEvent("set-edit","click",function(){
		...
		for(var i=0;i<elearr.length;i++){
			...
			if(flag){
				...
				//obj.nav.style.cssText=" ";
				obj.nav.style.backgroundColor=colorarr[Math.floor(Math.random()*5)];
